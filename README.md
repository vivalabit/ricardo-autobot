...
- `/find <item> [price_or_range] [delivery]` - parse live Ricardo.ch search results and return active listings. With a price/range or delivery-only phrase, Ricardo filters are applied.
- `/find -u <item> [price_or_range] [delivery]` - unique search: return only listings that were not shown in this chat's previous `/find` searches.
